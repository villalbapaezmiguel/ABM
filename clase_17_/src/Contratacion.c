...
/*contruye un espacio de memoria con las cantidad de bytes y me lo devuelve o retorna
 *malloc : va a reservar la cantidad de bytes que quiero recibir
 *retorna : un puntero
 *p : guarda la direccion de memoria del area que me devolvio malloc
 *return : si salio todo bien devuelve p ,  de lo contrario NULL*/
Contratacion* cli_new(void)
{
	//int* p <- tambien puede ser int , lo que pasa es que yo quiero tratar a esa area de memoria como si fuera un puntero a una veriable de tipo Contratacion
	//entonces guardo esa direccion porque asi puedo acceder a los campos y ya cargarle valores
	Contratacion* p = NULL;

	p = (Contratacion*)malloc(sizeof(Contratacion));//p apunta al espacio de memoria del cliente
...
//int cli_inicializarArrayPunteros(Contratacion** punterosContratacion , int lenContratacion)
{
	...
	if(punterosContratacion != NULL && lenContratacion > 0)
	{
		retorno = 1;
		for (int i = 0; i < lenContratacion; ++i) {

			punterosContratacion[i] = NULL;
		}
...
/**/
int cli_altaArrayConPunteros(Contratacion* contratacion , int len , int lugar)
{
	int retorno = -1;
	Pantalla bufferAux;
	...
	if(contratacion != NULL && len >0)
	{
		if(utn_getNombre(bufferAux.nombre, NOMBRE_LEN,"\nNombre cliente : ", "\nERROR ", 2) == 0 &&
		   utn_getNombre(bufferAux.direccion, DIRECCION_LEN, "\nIngrese su direccion : ", "\nERROR ", 2) == 0)
		{
			retorno = 1;
