...
{
	//clase 17 minuto : 30:00 lo explica mejor por si no estendes
//	Contratacion* pC;
	Contratacion* auxCliNew;
	Contratacion* punterosContratacion[lenContratacion];
	/*cuando el array es de tipo puntero lo unico que va a guardar va a ser un numero que es una direccion de memoria en donde estan todos los campos.
	 * (id, name , cuit , etc ) */

	cli_inicializarArrayPunteros(punterosContratacion , lenContratacion);
	cli_new();//recibimos la direccion de memoria en donde queremos dejar ese cliente
...
//	inicializar_contratacion(contratacion, lenContratacion);
	...
	while(opcion !=11)
	{
		...
		case 1:
			...
			if(cli_getEmptyIndexConArrayPunteros(punterosContratacion, lenContratacion, &lugarLibrePantalla)== 1)
			{
				auxCliNew = cli_new();

				if(auxCliNew != NULL)
				{
					//pedir datos al usuario y cargar los campos del espacio de memoria

					//guardar la direccion del nuevo cliente al nuevo puntero
					punterosContratacion[lugarLibrePantalla] = auxCliNew;
